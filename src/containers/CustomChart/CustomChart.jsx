...
import { ChartExample as Chart, getCustomConfig } from "../../components/Chart";
...
export default function CustomChart(props) {
	const storeRef = useRef(new CIQ.NameValueStore());
	const shortcutStorageName = "customDrawingToolShortcuts";
	const drawingToolsInfoRef = useRef(null);
	...
	const drawingToolDetails = {
		annotation: `Add text annotations to your chart.`,
		elliottwave: `The Elliott Wave Theory was developed by Ralph Nelson Elliott to describe...`
	};

	const configRef = useRef(null);

	if (!configRef.current) {
		configRef.current = CIQ.extend(getCustomConfig(props), props.config || {});

		// Inject Drawing Tools into Preferences menu (BEFORE chart init)
		configRef.current.menus.preferences.content = [
			...configRef.current.menus.preferences.content,
			{ type: "separator" },
	...
	const postInit = useCallback(({ chartEngine, uiContext }) => {
		if (props.chartInitialized) {
			props.chartInitialized({ chartEngine, uiContext });
		}

		updateCustomization(configRef.current).then(() => {
			addPreferencesHelper(uiContext);
			drawingToolsInfoRef.current = getDrawingTools(uiContext);
		});
	...
	const getValue = (name) => {
		return new Promise((resolve, reject) => {
			storeRef.current.get(name, (err, value) => {
	...
	const setValue = (name, value) => {
		return new Promise((resolve, reject) => {
			storeRef.current.set(name, value, (err) => {
				if (err) return reject(err);
				resolve(value);
			});
		});
	...
	// in localStorage
	const updateCustomization = async (cfg) => {
	...
	const addPreferencesHelper = (uiContext) => {
		const layoutHelper = uiContext.getAdvertised("Layout");
		layoutHelper.openPreferences = () => setShortcutDialog(true);
	};
	...
	const getDrawingTools = (uiContext) => {
		const details = drawingToolDetails;
		let drawingTools = configRef.current?.drawingTools.slice();
	...
	// localStorage
	const setDrawingToolShortcuts = (shortcuts) => {
		if (!uiContext) return;

		const { topNode } = uiContext;
		configRef.current.drawingTools.forEach((item) => {
			item.shortcut = shortcuts[item.tool];
		});
				...
				<Chart
					...
					<div className='ciq-nav full-screen-hide' role='navigation'>
						...
						<cq-toggle
							...
							icon='search'
							help-id='search_symbol_lookup'></cq-toggle>
						<cq-toggle
						...
						<cq-side-nav cq-on='sidenavOn'>
							<div className='icon-toggles ciq-toggles'>
								<cq-toggle
									...
									tooltip='Draw'
									icon='draw'
									help-id='drawing_tools_toggle'></cq-toggle>
								<cq-toggle
									class='ciq-CH'
									config='crosshair'
									reader='Crosshair'
									tooltip='Crosshair (Alt + \)'
									icon='crosshair'></cq-toggle>
								<cq-menu
									class='nav-dropdown toggle-options'
									reader='Crosshair Options'
									config='crosshair'></cq-menu>
								<cq-toggle
									class='ciq-HU'
									feature='tooltip'
									config='info'
						...
						<div className='ciq-menu-section'>
							<div className='ciq-dropdowns'>
								...
								<cq-menu
									...
									config='period'
									text
									binding='Layout.periodicity'></cq-menu>
								<cq-menu
								...
								<cq-menu
									...
									icon='events'
									responsive
									tooltip='Events'></cq-menu>
								<cq-menu
									class='nav-dropdown ciq-preferences alignright'
									reader='Preferences'
									config='preferences'
									icon='preferences'
									tooltip='Preferences'></cq-menu>
							</div>
					...
					<div className='ciq-chart-area' role='main'>
						<div className='ciq-chart'>
							<cq-message-toaster
								...
								default-position='top'></cq-message-toaster>
							...
							<cq-palette-dock>
								<div className='palette-dock-container'>
									<cq-drawing-palette
										...
										cq-keystroke-claim></cq-drawing-palette>
									<cq-drawing-settings
							...
							<div className='chartContainer'>
								...
								<cq-chart-title
									cq-marker
									cq-browser-tab
									cq-activate-symbol-search-on-click></cq-chart-title>
								...
								<cq-marker class='chart-control-group full-screen-show'>
									...
									<cq-toggle
										...
										reader='Crosshair'
										icon='crosshair'
										tooltip='Crosshair (Alt + \)'></cq-toggle>
									<cq-toggle
										class='ciq-DT'
										feature='tableview'
										member='tableView'
					...
					<div role='complementary' className='ciq-footer full-screen-hide'>
						...
						<cq-toggle
							...
							icon='help'
							tooltip='Interactive Help'></cq-toggle>
						<cq-show-range
							config='range'
							role='group'
							aria-labelledby='label_showRange'></cq-show-range>
					</div>
					...
					{shortcutDialog && (
						<ShortcutDialog
							drawingToolsInfo={drawingToolsInfoRef.current}
							closeDialog={() => setShortcutDialog(false)}
